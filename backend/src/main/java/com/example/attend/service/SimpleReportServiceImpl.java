...
/**
 * Implementation of SimpleReportService.
 * Uses JPA queries via EntityManager to fetch attendance data.
 */
@Service
public class SimpleReportServiceImpl implements SimpleReportService {
    ...
    /**
     * Returns a paginated list of attendance reports for a specific session.
     */
    @Override
    ...
    /**
     * Returns all attendance records from all sessions.
     * Used for CSV download (/api/report/all.csv)
     */
    @Override
    public List<AttendanceReportDTO> findAllReports() {
        String jpql =
            "SELECT new com.example.attend.dto.AttendanceReportDTO(" +
            "  a.studentUsername, a.studentId, a.studentName, a.submittedAt, " +
            "  a.lat, a.lng, a.accuracyMeters, a.ipTruncated, a.deviceHash, " +
        ...
        return em.createQuery(jpql, AttendanceReportDTO.class).getResultList();
    }

    /**
     * Utility to anonymize sensitive identifiers (username/studentId).
     */
